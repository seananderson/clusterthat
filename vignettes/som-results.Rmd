...
```{r data1_g3, fig.height = 3, fig.width = 4}
ggplot(simg3_mod) + geom_point(aes(
  x = bbmsy_median, y = ffmsy_median,
  colour = model_id, group = model_id
)) +
  scale_colour_viridis_c()
...
```{r data2_g3, fig.height = 3, fig.width = 4}
ggplot(simg3_mod) + geom_point(aes(
  x = fo_median, y = ffmsy_median,
  colour = model_id, group = model_id
)) +
...
```{r multi_g3, fig.height = 9, fig.width = 6}
clusterthat::plot_som(som_model,
   data = df, colour_vector = simg3_mod$fmodel,
   colour_label = "F model",
   nrow = 3
 )
```
...
```{r codes_g3, fight.height=5, fig.width = 6}
plot(som_model, type = "codes", shape = "straight")
```
