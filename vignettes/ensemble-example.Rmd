...
We will fit a model with 2 underlying trends. We will only use one chain so that this runs quickly but you will likely want to use at least 4 and probably more iterations.

```{r, warning=FALSE, results="hide"}
m2 <- fit_dfa(y = df, num_trends = 2, chains = 1, iter = 400, data_shape = "long")
```

We will apply a varimax rotation, as is commonly done with, for example, Principal Component Analysis and then plot the underlying trends:
