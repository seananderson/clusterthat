...
df <- add_features_flr(sim_mod, sim_ts) %>%
...
```{r}
factoextra::fviz_nbclust(df_scaled, cluster::pam, method = "gap_stat",
  k.max = 35)
```
...
```{r, warnings = FALSE}
m_pam <- cluster::pam(df_scaled, k = 17)
...
```{r, warnings = FALSE}
factoextra::fviz_nbclust(df_scaled, cluster::pam, method = "gap_stat",
  metric = "manhattan", k.max = 25)

m_pam_manhattan <- cluster::pam(df_scaled, k = 21, metric = "manhattan")
